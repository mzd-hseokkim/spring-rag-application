    ...
    <div className="flex h-screen bg-background">
      <aside className="w-80 bg-sidebar text-sidebar-foreground border-r border-sidebar-border flex flex-col">
        <Tabs defaultValue="conversations" className="flex flex-col flex-1 min-h-0">
          <TabsList className="mx-3 mt-3 bg-sidebar-accent/50">
            <TabsTrigger value="conversations" className="flex-1 text-xs text-sidebar-foreground/50 hover:text-sidebar-foreground data-active:bg-sidebar-primary data-active:text-sidebar-primary-foreground">
              대화
            </TabsTrigger>
            <TabsTrigger value="documents" className="flex-1 text-xs text-sidebar-foreground/50 hover:text-sidebar-foreground data-active:bg-sidebar-primary data-active:text-sidebar-primary-foreground">
              문서
            </TabsTrigger>
          </TabsList>
        ...
        <div className="border-t border-sidebar-border px-3 h-16 flex items-center gap-2">
          ...
          <ThemeToggle />
          {isAdmin && (
            <Button variant="ghost" size="icon" onClick={() => navigate('/admin')} className="shrink-0 text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/50" title="관리">
              <Settings2 className="h-4 w-4" />
            </Button>
          )}
          <Button variant="ghost" size="icon" onClick={logout} className="shrink-0 text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/50" title="로그아웃">
